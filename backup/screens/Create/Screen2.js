...
import { RNFFmpeg, RNFFmpegConfig, LogLevel } from 'react-native-ffmpeg';
...
export default function Screen2({ navigation }) {
    ...
    const [inpCommand , setInpCommand] = React.useState("")

    const executeFFmpeg = async(command) => {   await RNFFmpeg.execute(command);     }
    
    const saveImg = async() => {
        const res = await captureRef(viewRef, { quality: 1, format: "jpg" })
        for (var i = 0; i < text.length; i++) {
            //FileSystem.downloadAsync(res, FileSystem.cacheDirectory + "p" + i + ".jpg");
            MediaLibrary.saveToLibraryAsync(res)
        };

        RNFFmpegConfig.setLogLevel(LogLevel.AV_LOG_INFO);
        RNFFmpegConfig.registerNewFFmpegPipe();
        TestUtil.testParseArguments();
        setInpCommand("-i " + FileSystem.cacheDirectory + "p%d.jpg " + "/storage/emulated/0/1_imp/oout.mp4" );
        //executeFFmpeg(inpCommand)
        console.log(res);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: "#3b444b" }}>
            ...
            <View style={{ backgroundColor: "#000", height: 50, flexDirection: "row" }}>
                ...
                <TouchableOpacity style={{ marginLeft: 300, marginTop: 12 }} onPress={saveImg}>
